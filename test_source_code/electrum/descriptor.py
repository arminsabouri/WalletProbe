...
class PubkeyProvider(object):
    ...
    def __init__(
        ...
        if deriv_path:
            ...
            if wildcard_count > 1:
                raise ValueError(
                    "only one wildcard(*) is allowed in a descriptor")
            if wildcard_count == 1:
                if deriv_path[-1] != "*":
                    raise ValueError(
                        "wildcard in descriptor only allowed in last position")
            if deriv_path[0] != "/":
                raise ValueError(
                    f"deriv_path suffix must start with a '/'. got {deriv_path!r}")
        ...
        try:
            ...
            self.extkey = BIP32Node.from_xkey(
                pubkey, allow_custom_headers=False)
    ...
    def get_full_derivation_int_list(self, *, pos: Optional[int] = None) -> List[int]:
        ...
        if self.is_range() and pos is None:
            raise ValueError("pos must be set for ranged descriptor")
        path: List[int] = self.origin.get_full_int_list(
...
class Descriptor(object):
    ...
    def to_string_no_checksum(self) -> str:
        ...
        return "{}({}{})".format(
            ...
            self.subdescriptors[0].to_string_no_checksum() if len(
                self.subdescriptors) > 0 else ""
        )
    ...
    def expand(self, *, pos: Optional[int] = None) -> "ExpandedScripts":
        ...
        raise NotImplementedError(
            "The Descriptor base class does not implement this method")
...
class MultisigDescriptor(Descriptor):
    ...
    def __init__(
        ...
        if self.is_sorted:
            if not self.is_range():
                ...
                self.pubkeys = [x[1]
                                for x in sorted(zip(der_pks, self.pubkeys))]
            else:
    ...
    def expand(self, *, pos: Optional[int] = None) -> "ExpandedScripts":
        ...
        script = construct_script(
            [self.thresh, *der_pks, len(der_pks), opcodes.OP_CHECKMULTISIG])
        return ExpandedScripts(output_script=script)
    ...
    def _satisfy_inner(self, *, sigdata=None, allow_dummy=False) -> ScriptSolutionInner:
        ...
        assert len(
            signatures) == self.thresh, f"thresh={self.thresh}, but got {len(signatures)} sigs"
        return ScriptSolutionInner(
...
class SHDescriptor(Descriptor):
    ...
    def expand(self, *, pos: Optional[int] = None) -> "ExpandedScripts":
        ...
        script = construct_script(
            [opcodes.OP_HASH160, hash_160(redeem_script), opcodes.OP_EQUAL])
        return ExpandedScripts(
    ...
    def satisfy(self, *, sigdata=None, allow_dummy=False) -> ScriptSolutionTop:
        ...
        if isinstance(subdesc, (WSHDescriptor, WPKHDescriptor)):  # witness_v0 nested in p2sh
            witness = subdesc.satisfy(
                sigdata=sigdata, allow_dummy=allow_dummy).witness
            script_sig = construct_script([redeem_script])
        else:  # legacy p2sh
            subsol = subdesc._satisfy_inner(
                sigdata=sigdata, allow_dummy=allow_dummy)
            script_sig = construct_script(
                [*subsol.witness_items, redeem_script])
        return ScriptSolutionTop(
            witness=witness,
            script_sig=script_sig,
        )
...
class WSHDescriptor(Descriptor):
    ...
    def satisfy(self, *, sigdata=None, allow_dummy=False) -> ScriptSolutionTop:
        assert not self.is_range()
        assert len(self.subdescriptors) == 1
        subsol = self.subdescriptors[0]._satisfy_inner(
            sigdata=sigdata, allow_dummy=allow_dummy)
    ...
    def __init__(
        ...
        if desc_tree:
            if self.get_max_tree_depth() > MAX_TAPROOT_DEPTH:
                raise ValueError(
                    f"tr() supports at most {MAX_TAPROOT_DEPTH} nesting levels")
        ...
        super().__init__(
            ...
            # FIXME we could do without the flattened list (dupl)
    ...
    def expand(self, *, pos: Optional[int] = None) -> "ExpandedScripts":
        ...
        if self.desc_tree:
            def transform(tree_node):
                if isinstance(tree_node, Descriptor):
                    leaf_version = 0xc0
                    # FIXME maybe rename scriptcode_for_sighash
                    leaf_script = tree_node.expand(
                        pos=pos).scriptcode_for_sighash
...
def parse_pubkey(expr: str, *, ctx: '_ParseDescriptorContext') -> Tuple['PubkeyProvider', str]:
    ...
    permit_uncompressed = ctx in (
        _ParseDescriptorContext.TOP, _ParseDescriptorContext.P2SH)
...
def _parse_descriptor(desc: str, *, ctx: '_ParseDescriptorContext') -> 'Descriptor':
    ...
    if func == "sortedmulti" or func == "multi":
        ...
        if len(pubkeys) == 0 or len(pubkeys) > 15:
            raise ValueError(
                "Cannot have {} keys in a multisig; must have between 1 and 15 keys, inclusive".format(len(pubkeys)))
        elif thresh < 1:
            raise ValueError(
                "Multisig threshold cannot be {}, must be at least 1".format(thresh))
        elif thresh > len(pubkeys):
            raise ValueError("Multisig threshold cannot be larger than the number of keys; threshold is {} but only {} keys specified".format(
                thresh, len(pubkeys)))
...
def create_dummy_descriptor_from_address(addr: Optional[str]) -> 'Descriptor':
    ...
    desc = get_singlesig_descriptor_from_legacy_leaf(
        pubkey=pubkey, script_type=script_type)
    return desc
